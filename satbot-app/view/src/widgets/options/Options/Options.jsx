...
const Options = ({ options }) => {
  const runHandler = (handler, userID, sessionID, userInput, userInputType) => {
    ...
    // Check if the buttons are protocol buttons
    var protocol_num = NaN;
    var lang = "";
    if (isNaN(Number(userInput[0])) === false) {
      console.log(Number(userInput[0]))
      // 2 digit protocol number
      if (isNaN(Number(userInput[1])) === false) {
        protocol_num = userInput.substring(0,2);
      }
      else{
        protocol_num = userInput[0]
      };
      
      // Check the language of the protocol
      let alpha_numeric = isAlpha(userInput);

      if (alpha_numeric === false){
        lang = "ZH";
      }
      else{
        lang = "EN";
...
///////////// Functions used to display protocols in viewer /////////////
function isAlpha(str) {
    // Checks if str contains alphabets
